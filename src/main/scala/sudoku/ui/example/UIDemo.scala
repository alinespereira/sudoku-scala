    ...
    contents = new BorderPanel {
      ...
      val tabs: TabbedPane = new TabbedPane {
        ...
        val buttons: FlowPanel = new FlowPanel {
          ...
          contents += new BoxPanel(Orientation.Vertical) {
            border = CompoundBorder(
              TitledBorder(EtchedBorder, "Radio Buttons"),
              EmptyBorder(5, 5, 5, 10)
            )
          ...
          contents += new BoxPanel(Orientation.Vertical) {
            border = CompoundBorder(
              TitledBorder(EtchedBorder, "Check Boxes"),
              EmptyBorder(5, 5, 5, 10)
            )
        ...
        val password: FlowPanel = new FlowPanel {
          ...
          contents += label
          listenTo(field)
          reactions += { case EditDone(`field`) =>
            label.text = field.password.mkString
          }
        }
      ...
      val center: SplitPane =
        new SplitPane(Orientation.Vertical, new ScrollPane(list), tabs) {
