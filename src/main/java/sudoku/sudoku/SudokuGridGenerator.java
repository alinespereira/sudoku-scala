...
class SudokuGridGenerator {
  ...
  // // not including time it takes for print statements, which eats up BUTT-LOADS
  // of
  // // time and slows down the output 10-fold
  // if (isPerfect(sudoku.generateGrid()))
  // System.out.println("PERFECT GRID #" + String.format("%,d", (trials)));
  // }
  // }

  /**
   * Generates a valid 9 by 9 Sudoku grid with 1 through 9 appearing only once in
  ...
  public int[] generateGrid() {
    ...
    for (int i = 0; i < 9; i++) {
      ...
      for (int a = 0; a < 2; a++) {
        ...
        boolean[] registered = new boolean[10]; // index 0 will intentionally be left empty since there are only number
                                                // 1-9.
        int rowOrigin = i * 9;
        int colOrigin = i;

        ROW_COL: for (int j = 0; j < 9; j++) {
          // row/column stepping - making sure numbers are only registered once and
          // marking which cells have been sorted
            ...
            for (int y = j; y >= 0; y--) {
              ...
              if (grid[scan] == num) {
                ...
                for (int z = (a % 2 == 0 ? (i % 3 + 1) * 3 : 0); z < 9; z++) {
                  ...
                  if ((!sorted[scan] && !sorted[boxStep] && !registered[boxNum]) || (sorted[scan] && !registered[boxNum]
                      && (a % 2 == 0 ? boxStep % 9 == scan % 9 : boxStep / 9 == scan / 9))) {
                    grid[scan] = boxNum;
                    grid[boxStep] = num;
                  ...
                  } else if (z == 8) // if z == 8, then break statement not reached: no candidates available
                  {
                    ...
                    // loop of size 18 to prevent infinite loops as well. Max of 18 swaps are
                    // possible.
                    // at the end of this loop, if continue or break statements are not reached,
                    // then
                    // fail-safe is executed called Advance and Backtrack Sort (ABS) which allows
                    // the
                        ...
                        if (grid[pacing] == searchingNo) {
                          ...
                          for (int c = 1; c < 3 - (i % 3); c++) {
                            ...
                            if ((a % 2 == 0 && adjacentCell >= 81) || (a % 2 == 1 && adjacentCell % 9 == 0))
                              ...
                              if (i % 3 != 0 || c != 1 || blindSwapIndex[adjacentCell] || registered[adjacentNo])
                                adjacentCell -= decrement;
                            }
                            adjacentNo = grid[adjacentCell];
  ...
  /**
   * Tests an int array of length 81 to see if it is a valid Sudoku grid. i.e. 1
   * through 9 appearing once each in every row, column, and box
   * 
